...
class UserOnline
{
    ...
    {
        if ( $this->em->getRepository('UserOnline')
            ->findOneBy([
                'userId' => $user->getId()
            ])
        ) {
    ...
    {
        $online = $this->em->getRepository('UserOnline')
                    ->findOneBy([
                        'userId' => $user->getId()
                    ]);
        if (!$online) {
    ...
    public function getOnlineUsers(int $id, int $channel)
    {
        ...
        $usersOnline = $this->em->getRepository('UserOnline')
    ...
    private function deleteInactiveUsers(int $id, int $channel)
    {
        ...
        $this->em->getRepository('UserOnline')
