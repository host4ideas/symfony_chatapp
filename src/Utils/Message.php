...
class Message
{
    ...
    public function getMessagesInIndex(User $user)
    {
        ...
        $messages = $this->em->getRepository('Message')
    ...
    private function getMessagesAfterChangingChannel(int $channel, User $user)
    {
        ...
        $lastId = $this->em->getRepository('Message')
            ->getIdFromLastMessage();
    ...
    public function addMessageToDatabase(User $user, string $text): array
    {
        ...
        if ($special['userId'] == ChatConfig::getBotId()) {
            $originalUser = $user;
            $user = $this->em->find('User', ChatConfig::getBotId());
            $text = $special['text'];
        }
    ...
    {
        $channel = $this->session->get('channel');
        $status = $this->em->getRepository('Message')
            ->deleteMessage($id);
    ...
    private function createArrayToJson(\Entity\Message $message, User $user)
    {
