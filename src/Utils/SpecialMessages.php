...
class SpecialMessages
{
    ...
    private function priv(array $text, User $user): array
    {
        ...
        $secondUser = $this->em->getRepository('User')->findOneBy(['username' => $textSplitted[0]]);
        if (!$secondUser) {
            $text = $text[0] . ' ' .
    ...
    private function insertPw(User $user, User $secondUser, array $textSplitted): \Entity\Message
    {
    ...
    private function invite(array $textSplitted, User $user): array
    {
        ...
        $userToInvite = $this->em->getRepository('User')->findOneBy(['username' => $textSplitted[1]]);
        if (!$userToInvite) {
        ...
        $invite = $this->em->getRepository('Invite')->findOneBy([
    ...
    {
        $bot = $this->em->find('User', ChatConfig::getBotId());
